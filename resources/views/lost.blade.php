    ...
    <head>
        ...
        <meta charset="UTF-8">
        <title>Lost Items | I FOUND</title>
 ...
 @include('components.navbar')
    ...
    <section class="lost-hero">
        ...
        <div class="lost-action" style="display: flex; align-items: center; gap: 10px; width: 100%; max-width: 600px;">
            <form action="/lost" method="GET" style="display: flex; width: 100%; gap: 10px;">
            ...
            <a href="/report-lost" class="report-btn" style="white-space: nowrap;">
                Report
...
@if(auth()->check())
    ...
    @if(auth()->id() == $item->user_id)
        ...
        <!-- Owner -->
        <a href="/lost/{{ $item->id }}/edit" class="contact-btn">
            Edit
        </a>

        <form action="/lost/{{ $item->id }}" method="POST">
            @csrf
            @method('DELETE')
        ...
        <form action="{{ route('lost.status', $item->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <button type="submit" class="contact-btn" style="background-color: #28a745; color: white;" onclick="return confirm('Tandai sebagai selesai/ditemukan?')">
                Resolved
            </button>
        </form>
        @endif
    ...
    @else
        ...
        <form action="{{ route('bookmarks.toggle') }}" method="POST">
            ...
            @php $isBookmarked = auth()->user()->bookmarks()->where('bookmarkable_id', $item->id)->where('bookmarkable_type', \App\Models\LostItem::class)->exists(); @endphp
